...
namespace ghostCodes
{
    internal class DressGirl : EnemyAI
    {
        ...
        {
            if (!gcConfig.fixGhostGirlBreakers.Value || instance.currentBehaviourStateIndex == 1 || instance.timesChased != 1)
                return;

            if (Random.Range(0, 100) < gcConfig.ggVanillaBreakerChance.Value)
            {
